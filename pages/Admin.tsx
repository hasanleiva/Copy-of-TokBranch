...
export const Admin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!id || !title || !jsonName) {
      alert("ID, Title, and JSON Filename are required.");
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4 bg-gray-50 p-5 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1 px-1">Unique ID (e.g. story_intro)</label>
                <input 
                  value={id} onChange={e => setId(e.target.value)}
                  className="w-full bg-white rounded-xl p-3 text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all"
                  placeholder="Unique identifier for Firestore"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1 px-1">Display Title</label>
                <input 
              ...
              <div className="relative">
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1 px-1 flex items-center gap-1">
                  <FileJson size={10} /> Bunny JSON Filename
                </label>
                <input 
                  value={jsonName} onChange={e => setJsonName(e.target.value)}
                  className="w-full bg-white rounded-xl p-3 text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all"
                  placeholder="Ex: feed_1.json"
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 bg-gray-50 p-5 rounded-2xl border border-gray-100 shadow-sm">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Story Configuration (Bunny)</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1 px-1">Source URL (MP4/HLS)</label>
                <input 
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1 px-1">Description & Tags</label>
                <textarea 
                  value={desc} onChange={e => setDesc(e.target.value)}
                  className="w-full bg-white rounded-xl p-3 text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all h-24"
               ...
               <div>
                 ...
                 <div className="flex gap-2">
                    ...
                    <button 
                        type="button"
                        onClick={addBranch}
                        className="bg-black hover:bg-gray-800 p-2.5 rounded-lg text-white"
                    >
                        <Plus size={16} />
                    </button>
                 </div>
            ...
            <div className="space-y-2 mt-4">
              {branches.map((b, i) => (
                <div key={i} className="flex items-center justify-between bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <button type="button" onClick={() => removeBranch(i)} className="text-gray-300 hover:text-red-500 p-1">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
          ...
          <button 
            type="submit"
            className="w-full bg-[#fe2c55] hover:bg-[#e6264c] text-white font-black py-4 rounded-2xl shadow-lg active:scale-[0.98] transition-all uppercase tracking-widest text-sm"
          >
            Save Configuration
          </button>
        </form>
